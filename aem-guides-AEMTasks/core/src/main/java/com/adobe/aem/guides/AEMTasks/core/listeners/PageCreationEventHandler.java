package com.adobe.aem.guides.AEMTasks.core.listeners;
...
public class PageCreationEventHandler implements EventHandler {
    ...
    public void handleEvent(Event event) {
        ...
        if (path != null && path.endsWith("/jcr:content")) {
            ...
            params.put(ResourceResolverFactory.SUBSERVICE, "sasankr");

            try (ResourceResolver resolver = resolverFactory.getServiceResourceResolver(params)) {
                log.info("ResourceResolver obtained for service user 'sasankr'");
                ...
                if (pageResource != null) {
                    ...
                    if (templatePath != null && templatePath.startsWith("/conf/AEMTasks")) {
    ...
    private void startWorkflow(ResourceResolver resolver, String path) throws WorkflowException {
        ...
        if (wfSession != null) {
            ...
            WorkflowModel wfModel = wfSession.getModel("/var/workflow/models/task10");
            if (wfModel != null) {
                WorkflowData wfData = wfSession.newWorkflowData("JCR_PATH", path);
                wfSession.startWorkflow(wfModel, wfData);
                log.info("Workflow started for page: {}", path);
            } else {
                log.warn("Workflow model not found at path: /var/workflow/models/task10");
            }
        } else {
